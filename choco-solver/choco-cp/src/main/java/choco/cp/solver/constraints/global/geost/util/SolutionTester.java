		...
		while(objItr1.hasNext())
		{
			...
			while(objItr2.hasNext())
			{
				...
				if(oid1 != oid2)
				{
					...
					Vector<ShiftedBox> sb1 = stp.getShape(o1.getShapeId().getInf());
					Vector<ShiftedBox> sb2 = stp.getShape(o2.getShapeId().getInf());
					...
					for(int i = 0; i < sb1.size(); i++)
					{
						for(int j = 0; j < sb2.size(); j++)
						{
							...
							{
								if(!(
										((sb2.elementAt(j).getOffset(k) + o2.getCoord(k).getInf() >= sb1.elementAt(i).getOffset(k) + o1.getCoord(k).getInf()) &&
										(sb2.elementAt(j).getOffset(k) + o2.getCoord(k).getInf() < sb1.elementAt(i).getOffset(k) + o1.getCoord(k).getInf()  + sb1.elementAt(i).getSize(k)))
										||
										((sb2.elementAt(j).getOffset(k) + o2.getCoord(k).getInf() + sb2.elementAt(j).getSize(k) > sb1.elementAt(i).getOffset(k) + o1.getCoord(k).getInf()) && 
										(sb2.elementAt(j).getOffset(k) + o2.getCoord(k).getInf() + sb2.elementAt(j).getSize(k) <= sb1.elementAt(i).getOffset(k) + o1.getCoord(k).getInf() + sb1.elementAt(i).getSize(k)))
									)
								   )
								{
									intersect = false;
