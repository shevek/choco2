...
public class Setup {
     ...
     * @return number of domain variables
     */
...
//	}
	...
	public void print()
	{
		...
		while(itr.hasNext())
		{
			...
			LOGGER.log(Level.INFO, "object id: {0}",  id);
			LOGGER.log(Level.INFO, "    shape id: {0}",  o.getShapeId().getInf());
			for (int i = 0; i < cst.getDIM(); i++) {
				LOGGER.log(Level.INFO, "    Coords x{0} : {1}   {2}", new Object[]{i, o.getCoord(i).getInf(), o.getCoord(i).getSup()});
			}
		}
		...
		while(itr.hasNext())
		{
			...
			LOGGER.log(Level.INFO, "shape id: {0}", sid);
			for(int i = 0; i < sb.size(); i++)
			{
				...
				LOGGER.log(Level.INFO, "    sb{0} : ", i);
				LOGGER.log(Level.INFO, "       Offset: {0}", offset);
