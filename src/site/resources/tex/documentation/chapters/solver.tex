...
The \mylst{Solver}, along with the \mylst{Model}, is one of the two key elements of any Choco program. The Choco \mylst{Solver} is mainly focus on resolution part: reading the \mylst{Model}, defining the search strategies and the resolution policy.  
...
One can access to the Solver representation of a Model constraint, using the \texttt{Solver} API \mylst{getCstr(Constraint c)}.
...
Setting \textit{size} to \mylst{Integer.MAX\_VALUE} means "store every solution", otherwise you can set a finite value, for example 10 and the $10^{th}$ last solutions will be stored.

A Solution object is made of arrays of values: an array of values of \mylst{IntVar}, another one for values of \mylst{SetVar} and the last one for the values of \mylst{RealVar}. For each type of variable, the $i^{th}$ one in the solver corresponds to the $i^{th}$ one in the solution:
...
A logging class is instrumented in order to produce trace statements throughout search: \mylst{ChocoLogging}. 
...
\begin{note}
Reusing a \mylst{Solver} must be prepared and well though. A backup of the initial state of the \mylst{Solver} can be necessary to recover initial domains and constraints internal structures. In the case where new constraints are created and added, this must be done manipulating \mylst{Solver} objects (SConstraint for example). So, this required knowledges in advanced uses of \mylst{Solver}.
\end{note}


When a \mylst{Solver} needs to be reused, there are some questions about what needed to be initialized.
\begin{itemize}
\item \underline{reset the search strategy :} this is done by calling \mylst{resetSearchStrategy()} on a \mylst{CPSolver}.
A call to this method clears the defined branching strategies (removes properly previous ones) and sets the current search strategy to \mylst{null}.
...
\subsection{Things to know on \mylst{Solver} reusability}

There are few things to know on a \mylst{Solver} reusability:
\begin{itemize}
\item a variable cannot be removed from a \mylst{Solver},
\item a statically posted constraint (added using \mylst{solver.postCut(SConstraint c)}) can be removed \textit{whenever} using \mylst{void eraseConstraint(SConstraint c)},
