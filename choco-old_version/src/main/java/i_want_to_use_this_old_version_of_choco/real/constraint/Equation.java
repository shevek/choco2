...
public class Equation extends AbstractLargeRealConstraint {
  ...
  public Equation(AbstractProblem pb, RealVar[] collectedVars, RealExp exp, RealInterval cste) {
    super(collectedVars);
    initEquation(pb, exp, cste);
  }

  public Equation(AbstractProblem pb, RealVar[] collectedVars, RealExp exp) {
    this(pb, collectedVars, exp, new RealIntervalConstant(0.0, 0.0));
  }
  ...
  public Object clone() throws CloneNotSupportedException {
    ...
    newc.initEquation(this.problem, this.exp, this.cste);
    return newc;
  }

  public void initEquation(AbstractProblem pb, RealExp exp, RealInterval cste) {
    this.problem = pb;
    this.cste = cste;
    this.exp = exp;
  ...
  public void addBoxedVar(RealVar var) {
    if (nbBoxedVars == boxedVars.length) {
      Logger.getLogger("dev.i_want_to_use_this_old_version_of_choco.prop.const").severe("Cannot box more variables than variables involved in the constraint !!");
      return;
    }
    ...
    if (wx.size() == 0) {
      Logger.getLogger("dev.i_want_to_use_this_old_version_of_choco.prop.const").severe("Cannot box variables not involved in the constraint !!");
      return;
    }
...
public boolean isSatisfied() {
    boolean ok = true;
    this.getProblem().getEnvironment().worldPush();
    try {
      this.propagate();
    } catch (ContradictionException e) {
      ok = false;
    }
    this.getProblem().getEnvironment().worldPop();
    return ok;
  }
