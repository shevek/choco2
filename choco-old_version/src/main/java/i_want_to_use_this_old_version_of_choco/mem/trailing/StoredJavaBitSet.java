...
public class StoredJavaBitSet implements IStateBitSet {
    /*
    ...
    */
     ...
     */
    private IStateBool sizeIsSticky;
    ...
    public StoredJavaBitSet(IEnvironment environment, int size, boolean fixed) {
        this(environment,size);
        sizeIsSticky.set(fixed);
    }

    /**
     * Creates a new bit set. All bits are initially <code>false</code>.
     */
    public StoredJavaBitSet(IEnvironment environment) {
        this.environment = environment;
        this.sizeIsSticky = (StoredBool) environment.makeBool(false);
        this.wordsInUse = (StoredInt) environment.makeInt(0);
     ...
     *                                    is negative.
     */
    public StoredJavaBitSet(IEnvironment environment, int nbits) {
        this.environment = environment;
        this.sizeIsSticky = environment.makeBool(false);
    ...
    public IntIterator getCycleButIterator(int avoidIndex) {
        ...
        if (n > 0) {
            return new StoredJavaBitSet.CyclicIterator(this, avoidIndex);
        } else {
            return new StoredJavaBitSet.EmptyIterator();
        }
    }
     ...
     * @since 1.4
     */
    public StoredJavaBitSet get(int fromIndex, int toIndex) {
        checkRange(fromIndex, toIndex);
        ...
        if (len <= fromIndex || fromIndex == toIndex)
            return new StoredJavaBitSet(environment, 0);

        // An optimization
        if (toIndex > len)
            toIndex = len;

        StoredJavaBitSet result = new StoredJavaBitSet(environment, toIndex - fromIndex);
     ...
     * @since 1.4
     */
    public boolean intersects(StoredJavaBitSet set) {
     ...
     * @param set a bit set.
     */
    public void and(StoredJavaBitSet set) {
        if (this == set)
            return;
     ...
     * @param set a bit set.
     */
    public void or(StoredJavaBitSet set) {
        if (this == set)
            return;
     ...
     * @param set a bit set.
     */
    public void xor(StoredJavaBitSet set) {
        int wordsInCommon = Math.min(wordsInUse.get(), set.wordsInUse.get());
     ...
     * @since 1.2
     */
    public void andNot(StoredJavaBitSet set) {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof StoredJavaBitSet))
            return false;
        if (this == obj)
            return true;

        StoredJavaBitSet set = (StoredJavaBitSet) obj;
    ...
    public Object clone() {
        ...
        StoredJavaBitSet result = new StoredJavaBitSet(environment, this.size());
    ...
    public static void main(String[] args) throws Exception{
        ...
        StoredJavaBitSet a = new StoredJavaBitSet((EnvironmentTrailing) pb.getEnvironment(), 5);
        StoredJavaBitSet b = new StoredJavaBitSet((EnvironmentTrailing) pb.getEnvironment(), 5);
        StoredJavaBitSet c = new StoredJavaBitSet((EnvironmentTrailing) pb.getEnvironment(), 5);
        StoredJavaBitSet d = (StoredJavaBitSet) b.clone();
        a.set(0, 4, true);
        b.set(3, 5, true);
        c.set(0, 4, true);
        System.out.println(a.toString() + " -- " + b.toString() + " -- " + c.toString() + " -- " + d.toString());
        System.out.println("world push");
        pb.worldPush();
        b.and(c);
        d.or(c);
        System.out.println(a.toString() + " -- " + b.toString() + " -- " + c.toString() + " -- " + d.toString());
        ...
        IntDomainVar[] vs = pb.makeEnumIntVarArray("x",4,1,10000000);

        pb.post(pb.neq(vs[0],vs[2]));
        pb.post(pb.neq(vs[1],vs[3]));
        pb.solve();


        for (IntDomainVar v : vs)
