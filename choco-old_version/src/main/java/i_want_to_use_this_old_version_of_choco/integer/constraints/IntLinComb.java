...
//
  ...
  public AbstractConstraint opposite() {
    ...
    IntExp term = pb.scalar(coeffs, vars);
    if (op == EQ) {
      return (AbstractConstraint) pb.neq(term, -cste);
    } else if (op == NEQ) {
      return (AbstractConstraint) pb.eq(term, -cste);
    } else if (op == GEQ) {
      return (AbstractConstraint) pb.lt(term, -cste);
    } else {
      return null;
    }
  }
   ...
   * @param compareTo the constraint to compare with the current one
   * @return true if the two constraints have the same supports
   */
  // TODO: we need to sort the variables and coefficients
  public boolean isEquivalentTo(final Constraint compareTo) {
    if (compareTo instanceof IntLinComb) {
      IntLinComb c = (IntLinComb) compareTo;
      int n = this.getNbVars();
      if (c.getNbVars() == n) {
        boolean allEqual = (this.op == c.op);
        for (int varIdx = 0; varIdx < n; varIdx++) {
          if ((this.getIntVar(varIdx) != c.getIntVar(varIdx))
          || (this.coeffs[varIdx] != c.coeffs[varIdx])) {
            allEqual = false;
            break;
          }
        }
