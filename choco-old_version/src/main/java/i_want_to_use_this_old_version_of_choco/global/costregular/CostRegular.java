 ...
 * Time: 1:38:31 PM
 */
public class CostRegular extends AbstractLargeIntConstraint
{
    /**
    ...
    private HashMap<Integer,State>[] layer;


    /**
     * Stack of the Arc whose vertices must be re-checked
     */
    public Queue<Arc> toVisit;
     ...
     */
    @SuppressWarnings("unchecked")
    private CostRegular(IntDomainVar[] vars, Automaton auto, int[][] costs)
    {
        super(vars);
        this.automaton = auto;
     ...
     */
    private void addToQij(int i, int j, State s)
    {
     ...
     */
    private void remFromQij(int i, int j, State s)
    {
        remove(getQij(i,j),s);
    }
    ...
    private void initGraph() throws ContradictionException
    {
        ...
        {
            layer[i] = new HashMap<Integer,State>(nbNodes);
        }

        source = new State(0,this.automaton.getStartingState(),this);
        source.pccS.set(0);
        source.pgcS.set(0);
    ...
    */
    ...
    private void printAll(State s, HashSet<Arc>[] map)
    {
     ...
     * @throws ContradictionException if a domain gets empty
     */
    private void updateCostBound() throws ContradictionException
    {
        for (State s : layer[0].values())
        {
            int pgcP = s.pgcP.get();
            if(pgcP < cVar.getSup())
                cVar.updateSup(pgcP,cIndices[myVars.length]);
        }
        for (State s : layer[myVars.length+1].values())
        {
            int pccS = s.pccS.get();
            if(pccS > cVar.getInf())
                cVar.updateInf(pccS,cIndices[myVars.length]);
        }
    ...
    public class State
    {
        ...
        private Arc remFromQueue()
        {
            Arc tmp = cr.toVisit.remove();
            tmp.inQueue = false;
            return tmp;
        }
    ...
    public static void main(String[] args) {
        ...
        IntDomainVar[] vars = pb.makeEnumIntVarArray("x",50,0,9);
        IntDomainVar cost = pb.makeBoundIntVar("z",0,Integer.MAX_VALUE);
        ...
        IntDomainVar[] pbv = new IntDomainVar[vars.length+1];
        System.arraycopy(vars,0,pbv,0,vars.length);
        pbv[vars.length] = cost;

        //pb.getSolver().setVarSelector(new StaticVarOrder(vars));
        CostRegular c = new CostRegular(pbv,new Automaton("(5)+(6)+7+"),costs);
        ...
        if (pb.solve()/*pb.maximize(cost,false)*/) {
            do {
                nbSol++;
                System.out.println(c.isSatisfied());
                for (int i = 0 ; i < pbv.length -1 ; i++)
                    System.out.print(pbv[i].getVal()+" ");
                System.out.println("");
                for (int i = 0 ; i < pbv.length -2 ; i++)
                    System.out.print((int)costs[i][pbv[i].getVal()]+" ");
                System.out.println("   cost : "+cost.getVal()+"       "+nbSol);
                lastcost = cost.getVal();
        ...
        System.out.println("NBNODE : "+pb.getSolver().getSearchSolver().getNodeCount());
