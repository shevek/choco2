...
public abstract class AbstractBipartiteGraph extends AbstractLargeIntConstraint {
  protected Logger logger = Logger.getLogger("dev.i_want_to_use_this_old_version_of_choco.prop.const");
  ...
  protected void init(int nbLeft, int nbRight) {
    ...
    this.refMatch = this.getProblem().getEnvironment().makeIntVector(this.nbLeftVertices, -1);
    this.matchingSize = this.getProblem().getEnvironment().makeInt(0);
