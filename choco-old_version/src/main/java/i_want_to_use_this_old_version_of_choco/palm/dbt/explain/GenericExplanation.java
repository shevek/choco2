...
import i_want_to_use_this_old_version_of_choco.palm.dbt.integer.explain.*;
...
import i_want_to_use_this_old_version_of_choco.palm.real.explain.RealDecSupExplanation;
import i_want_to_use_this_old_version_of_choco.palm.real.explain.RealIncInfExplanation;
...
public class GenericExplanation implements PalmExplanation {
  ...
  public void addAll(Collection collection) {
    for (Iterator iterator = collection.iterator(); iterator.hasNext();) {
      AbstractConstraint c = (AbstractConstraint) iterator.next();
  ...
  public boolean isValid(int time) {
    for (int i = explanation.nextSetBit(0); i >= 0; i = explanation.nextSetBit(i + 1)) {
      if (this.pb.getConstraintNb(i) == null || !((Propagator) this.pb.getConstraintNb(i)).isActive() ||
          ((PalmConstraintPlugin) ((AbstractConstraint) this.pb.getConstraintNb(i)).getPlugIn()).getTimeStamp() > time)
        return false;
    }
    //for (Iterator iterator = explain.iterator(); iterator.hasNext();) {
    //    IConstraint constraint = (IConstraint) iterator.next();
  ...
  public void clear() {
    ...
    for (int i = cp.nextSetBit(0); i >= 0; i = cp.nextSetBit(i + 1)) {
      if (((PalmConstraintPlugin) ((AbstractConstraint) this.pb.getConstraintNb(i)).getPlugIn()).isIndirect()) {
  ...
  public void addDependencies() {
    ...
    for (int i = this.explanation.nextSetBit(0); i >= 0; i = this.explanation.nextSetBit(i + 1)) {
      ((PalmConstraintPlugin) ((AbstractConstraint) this.pb.getConstraintNb(i)).getPlugIn()).addDependency(this);
    }
  }
...
((PalmConstraintPlugin)constraint.getPlugIn()).addDependency(this);
  ...
  public void removeDependencies(Constraint removed) {
    for (int i = this.explanation.nextSetBit(0); i >= 0; i = this.explanation.nextSetBit(i + 1)) {
      AbstractConstraint constraint = (AbstractConstraint) this.pb.getConstraintNb(i);
  ...
  public Set toNogood() {
    Set ret = new HashSet();
    for (int i = explanation.nextSetBit(0); i >= 0; i = explanation.nextSetBit(i + 1)) {
      if (((PalmConstraintPlugin) ((AbstractConstraint) (pb.getConstraintNb(i))).getPlugIn()).getWeight() == 0) ret.add(pb.getConstraintNb(i));
    }
    return ret;
  }
  ...
  public SymbolicDecision[] getNogood() {
    ...
    for (int i = explanation.nextSetBit(0); i >= 0; i = explanation.nextSetBit(i + 1)) {
      if (((PalmConstraintPlugin) ((AbstractConstraint) (pb.getConstraintNb(i))).getPlugIn()).getWeight() == 0)
        nogood.add(pb.getConstraintNb(i));
    }
    DecisionConstraint[] nogoodToReturn = new DecisionConstraint[nogood.size()];
