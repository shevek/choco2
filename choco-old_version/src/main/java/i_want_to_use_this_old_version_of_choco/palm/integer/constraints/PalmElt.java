...
public class PalmElt extends AbstractPalmBinIntConstraint {
  ...
  public PalmElt(final IntDomainVar v0, final IntDomainVar v1,
    ...
    this.cste = cste;
    this.lvals = lvals;
    this.hook = ((ExplainedProblem) this.getProblem()).
    ...
    if (v0.hasEnumeratedDomain()) {
      for (int i = minFeasibleIndex + 1; i < maxFeasibleIndex; i++) {
        ...
        && !v1.canBeInstantiatedTo(lvals[i + cste])) {
          Explanation expl =
              ((ExplainedProblem) this.getProblem()).makeExplanation();
   ...
   * @throws i_want_to_use_this_old_version_of_choco.ContradictionException if a contradiction occurs or a domain
   * becomes empty.
   */
  ...
  public void awakeOnRestore(final int idx) throws ContradictionException {
    Explanation expl = ((ExplainedProblem) this.getProblem()).
        makeExplanation();
    ((ExplainedConstraintPlugin) this.hook).self_explain(expl);
